...
ChinoView.extend = function() {
  var SPECIAL_PROPS = ['basePath', 'name', 'template', 'templates', 'specials', 'elements', 'uiEvents', 'chinoEvents'],
      i;
  ...
  //Manipulate Chino Events
  if(NewViewAttrs._chinoEvents && this._chinoEvents) {
    for (var key in NewViewAttrs._chinoEvents) {
      if (this._chinoEvents[key]) {
        NewViewAttrs._chinoEvents[key] = this._chinoEvents[key] + ', ' + NewViewAttrs._chinoEvents[key];
      }
    }
  }
